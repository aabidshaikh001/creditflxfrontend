...
export default function ApplyPage() {
  ...
  useEffect(() => {
    ...
    socket = io("http://localhost:7000"); // Update this with your server's URL
...
socket.on("showForm", (data: { feeId: string; formType: string }) => {
  ...
  useEffect(() => {
   ...
   useEffect(() => {
    const fetchUserDetails = async () => {
      try {
        const response = await axios.get("http://localhost:7000/api/bank-details/user-details");
  ...
  const fetchQrCode = async () => {
    try {
      const response = await axios.get("http://localhost:7000/api/get-qr");
  ...
  const handleNext = async (): Promise<void> => {
    ...
    if (step === 2) {
      ...
      try {
        ...
        const response = await axios.post("http://localhost:7000/api/user-details/save", formDataToSend, {

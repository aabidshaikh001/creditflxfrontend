...
export default function ApplyPage() {
  ...
  useEffect(() => {
    ...
    socket = io("https://backend.creditsfix.in"); // Update this with your server's URL
...
socket.on("showForm", (data: { feeId: string; formType: string }) => {
  ...
  useEffect(() => {
   ...
   useEffect(() => {
    const fetchUserDetails = async () => {
      try {
        const response = await axios.get("https://backend.creditsfix.in/api/bank-details/user-details");
  ...
  const fetchQrCode = async () => {
    try {
      const response = await axios.get("https://backend.creditsfix.in/api/get-qr");
  ...
  const handleNext = async (): Promise<void> => {
    ...
    if (step === 2) {
      ...
      try {
        ...
        const response = await axios.post("https://backend.creditsfix.in/api/user-details/save", formDataToSend, {
